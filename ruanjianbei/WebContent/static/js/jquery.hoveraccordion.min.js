...
(function($) {
	...
	$.fn.hoverAccordion = function(opts) {
			...
			if (false === oldList.is(":animated")) {
				...
				if (itemHeader.hasClass(options.classOpen) === false) {
					...
					oldList.closest("li").removeClass(options.classActive)
							.find("a:first").addClass(options.classClosed)
		...
		$(this)
				...
				.each(
						function() {
							var item = $(this), isActive = false, itemHeader = item
									.find("a:first").addClass(
											options.classHeader), itemHref, itemList, listHeight;
							itemNo++;
							if (itemHeader.length > 0) {
								...
								if (itemHref === "#") {
									itemHeader.click(function() {
										...
										&& pageHref.length
												- pageHref
														.lastIndexOf(itemHref) === itemHref.length) {
									isActive = true;
